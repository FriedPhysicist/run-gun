...
public class charr_ : MonoBehaviour
{
    ...
    [Header("Camera")] public float shake_duration;
    ...
    float distance_betweeen_e=5.5f;
    private bool _idle=false;
    private bool death_=false;

    public Transform stuck_parrent;
    public GameObject stuck_point;
    ...
    void Start()
    {
        ...
        cm_noise = cm.GetCinemachineComponent<Cinemachine.CinemachineBasicMultiChannelPerlin>();
    ...
    void Update()
    {
        ...
        while (c_enemy!=null)
        {
            ...
            {
                distance_betweeen_e = 5.5f;
            }

            if (c_enemy.CompareTag("enemy_2"))
            {
                distance_betweeen_e=4.5f;
            }
    ...
    void FixedUpdate()
    {
        ...
        //if jump_attack==true then add local velocity to roll forward with animation
        while (jump_attack)
        {
            if (dynamic_distance<6)
            {
                jump_attack = false;
                break;
            }
    ...
    void fight_things()
    {
        ...
        if(touch.phase==TouchPhase.Moved && !afight_bool && !death_)
        {
            _idle = false;
    ...
    {
        //second_chance
        //when die make every movement boolean false to stop
        if (healt_<=0)
        {
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.CompareTag("enemy"))
        {
            ...
            if (c_enemy.GetComponent<mix_enemy>().hit_event && !afight_bool)
            {
                anim_holder = "impact";
                anim.SetBool(anim_holder,true);
                if (current_letter>=1)
                ...
                afight_bool = false;
                c_enemy.GetComponent<mix_enemy>().hit_event = false;
            }
        }
    ...
    void score_canvas_handler()
    {
        ...
        if (score_number_show% 3==0 && score_number_show>0 )
        {
            pause_bool = true;
    ...
    //for protect the function to execute for once a tiem
    void event_hit()
    {
        event_attack = true;
    }
