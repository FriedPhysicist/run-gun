...
public class monster_script_little : MonoBehaviour
{
    ...
    public float speed;
    bool can_run=true;
    private float distance_betweeen_e = 12f;
    ...
    void Update()
    {
        target = GameObject.FindGameObjectWithTag("valk");
        ...
        if (_animator.GetCurrentAnimatorStateInfo(0).IsName("impact") && !valkriye_.event_attack&& health>0)
        {
    ...
    void air_fight(bool get_acitve)
    {
        while (get_acitve)
        {
    ...
    void fight_and_death()
    {
        ...
        if (distance_b_player >= distance_betweeen_e && health>0)
        {
            can_run = true;
            anim_bool = false;
        }
    ...
    {
        if(other.CompareTag("valk") && valkriye_.event_attack && for_once && valkriye_.afight_bool && health>0)
        {
            for_once = false;
            hit_value();
        }
    ...
    void run()
    {
        ...
        if (!can_run)
        {
            ...
            rb.velocity = Vector3.zero+new Vector3(0,-speed,0);
        }
    ...
    void audio_event()
    {
        ...
        if (audio_protector && !valkriye_.afight_bool && distance_b_player<=15f)
        {
            sfx.clip =hit_sound;
            valkriye_.impact_boolean = true;
            valkriye_.healt_ -= valkriye_.get_damage;
